...
function buildDefaultKeyframes(): Keyframe[] {
  ...
  return [
    ...
    { t: S * 7,   pos: [0, 0, 0], look: [0, 0, 0], planet: 4 },
    { t: S * 8,   pos: [SC.x - 8, 14, SC.z + 25], look: [SC.x, 0, SC.z], planet: -1 },
...
function computeKeyframes(): Keyframe[] {
  ...
  for (let i = 0; i < kf.length; i++) {
    ...
    const el = document.getElementById(`world-${k.planet}`);
    if (!el) continue;
  ...
  const lastPlanetIdx = kf.findIndex((k) => k.planet === 4);
  if (lastPlanetIdx >= 0 && lastPlanetIdx + 1 < kf.length) {
    const lastEl = document.getElementById("world-4");
    if (lastEl) {
      const bottom =
