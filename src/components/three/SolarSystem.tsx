...
export const planetPositions: THREE.Vector3[] = Array.from(
...
const PLANET_CONFIGS: PlanetConfig[] = [
  ...
  { orbitRadius: 15, orbitSpeed: 0.035, size: 1.55, texture: "/textures/2k_jupiter.webp", atmosphere: "#ddbb77", twilight: "#aa7744", tilt: 0.05, hasRing: false, isEarth: false },
  { orbitRadius: 20, orbitSpeed: 0.02, size: 1.25, texture: "/textures/2k_saturn.webp", atmosphere: "#eedd88", twilight: "#aa8833", tilt: 0.47, hasRing: true, isEarth: false },
  { orbitRadius: 25, orbitSpeed: 0.014, size: 1.05, texture: "/textures/2k_mars.webp", atmosphere: "#b78cff", twilight: "#6540aa", tilt: 0.25, hasRing: false, isEarth: false },
];
...
function EarthPlanet({ config, index }: { config: PlanetConfig; index: number }) {
  ...
  useFrame(({ clock }) => {
    ...
    const angle = clock.elapsedTime * config.orbitSpeed + index * (Math.PI * 2) / worlds.length;
    ...
    <group ref={groupRef}>
      ...
      {worlds.map((world, i) => {
        ...
        return config.isEarth
          ? <EarthPlanet key={world.id} config={config} index={i} />
          : <RockyPlanet key={world.id} config={config} index={i} />;
      })}
    </group>
  );
}
